...
router.post('/', requireRole('admin'), (req, res) => {
  try {
    ...
    // BantayPlaka: only admin and guard. Admin creates guard users via UI.
    const userRole = role === 'guard' ? 'guard' : 'guard';
...
router.put('/:id', requireRole('admin'), (req, res) => {
  try {
    ...
    // Only allow updating guard; cannot change admin via UI
    let updateRole = existingUser.role;
    if (role === 'guard' && existingUser.role !== 'admin') {
      updateRole = 'guard';
    }
