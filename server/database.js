...
async function initDatabase() {
  ...
  // Migrate existing users table to add role column if needed (BantayPlaka: admin | guard only)
  try {
    db.run('ALTER TABLE users ADD COLUMN role TEXT NOT NULL DEFAULT \'guard\'');
  } catch (error) {
    const errorMsg = error?.message || String(error);
    if (!errorMsg.includes('duplicate column name') && !errorMsg.includes('no such table')) {
  ...
  // Normalize roles: only admin and guard (migrate from old encoder/barangay_user)
  try {
    db.run(`UPDATE users SET role = 'guard' WHERE role IS NULL OR role = '' OR role NOT IN ('admin', 'guard')`);
  } catch (error) {
    // Ignore
  }
