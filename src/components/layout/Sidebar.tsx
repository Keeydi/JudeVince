...
function NavContent({ collapsed, onNavigate }: { collapsed?: boolean; onNavigate?: () => void }) {
  ...
  // Guard: dashboard, vehicles, hosts, cameras, upload, warnings, capture results, violations, analytics, settings. Admin: all + User Management, Audit Logs.
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="left" className="w-64 p-0 bg-card border-border">
        ...
        <SheetDescription className="sr-only">
          Main navigation menu for BantayPlaka application
        </SheetDescription>
        <div className="flex h-full flex-col">
          {/* Logo */}
          <div className="flex h-16 items-center justify-between border-b border-border px-4">
            <div className="flex items-center gap-2">
              <img src="/logo.png" alt="BantayPlaka" className="h-8 w-8" />
              <span className="font-semibold text-lg">BantayPlaka</span>
            </div>
          </div>
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="flex h-16 items-center justify-between border-b border-border px-4">
            ...
            {collapsed && <img src="/logo.png" alt="BantayPlaka" className="h-8 w-8 mx-auto" />}
          </div>
